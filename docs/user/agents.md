...
| OpenHands | `oh` | SDK final assistant/finish text; native headless autoapproval; no model/effort override; accounting unknown |
| Warp | `wp` | Legacy executable `oz`; NDJSON agent text; model override, no effort or accounting |
| iFlow | `if` | Legacy text mode; existing BYOK setup required; model override, no effort or accounting |
| Qwen Code | `qw` | JSONL final result, token counts and root-message model IDs; model and native session-turn limit |
| Amp | none | JSONL final result and optional token counts; no generic model, effort or budget override |
| Reasonix | `rx` | Whole JSON final result and native token counts; configured provider selector and provider-specific effort |
| Droid | `dr` | Whole JSON result; model and provider-specific reasoning effort; accounting unknown |
| Kimi CLI | `km` | Final-only assistant JSONL; native autoapproval; model override; accounting unknown |
| Mistral Vibe | `mv` | Whole public-history JSON; native turns/USD limits; no model/effort override or accounting |
| Crush | `cr` | Quiet text capture; local run autoapproval; model override; accounting unknown |
| Devin | `dv` | Print text capture; requires trusted workspace; model override; accounting unknown |
| Cortex Code / CoCo | `co` | Exec text capture; Snowflake connection required; model/effort/turns; accounting unknown |
