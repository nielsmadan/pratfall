...
def run(invocation: Invocation, cwd: Path, timeout: float) -> ProcessResult:
    ...
    try:
        try:
            with tempfile.TemporaryFile() as child_input:
                ...
                return _collect(process, started, timeout, signal_state)
        except OSError as error:
...
def _collect(
    ...
    try:
        error, timed_out = _drain(process, selector, buffers, deadline, signal_state)
        if signal_state.received is not None or timed_out or error is not None:
            try:
                _terminate_and_drain(process, selector, buffers, signal_state)
...
def _drain(
    ...
    while True:
        ...
        for key, _ in selector.select(max(0.0, wait)):
            read_error = _read_ready(selector, key, buffers)
...
def _terminate_and_drain(
    ...
    while time.monotonic() < grace_deadline and not signal_state.repeated:
        for key, _ in selector.select(0.05):
            _read_ready(selector, key, buffers)
        process.poll()
